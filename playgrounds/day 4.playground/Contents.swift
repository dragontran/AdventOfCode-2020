...
let passwordsWithValidFields = passports.filter { passport in
    return values.allSatisfy { field in
        return passport.contains(field)
    }
}
count = passwordsWithValidFields.count
print(count)

//
func test(numberString: String, max: Int, min: Int) -> Bool {
...
// part 2
count = 0
passportLoop: for passport in passwordsWithValidFields {
    let components = passport.components(separatedBy: CharacterSet(charactersIn: " \n"))
    for component in components {
        let tests = component.components(separatedBy: ":")
//        print(tests)
        switch tests[0] {
            case "byr":
                if tests[1].count != 4 {
//                    print("fail")
                ...
                guard test(numberString: tests[1], max: 2002, min: 1920) else {
//                    print("fail")
                    continue passportLoop
                }
...
//                    print("fail")
                ...
                guard test(numberString: tests[1], max: 2020, min: 2010) else {
//                    print("fail")
                    continue passportLoop
                }
...
//                    print("fail")
                ...
                guard test(numberString: tests[1], max: 2030, min: 2020) else {
//                    print("fail")
                    continue passportLoop
                }
...
//                print("pass")
            case "hgt":
                if tests[1].contains("cm") {
                    let string = tests[1].replacingOccurrences(of: "cm", with: "")
                    guard test(numberString: string, max: 193, min: 150) else {
//                        print("fail")
                        continue passportLoop
                    }
                } else if tests[1].contains("in") {
                    let string = tests[1].replacingOccurrences(of: "in", with: "")
                    guard test(numberString: string, max: 76, min: 59) else {
//                        print("fail")
                        continue passportLoop
                    }
                } else {
...
//                print("pass")
            case "hcl":
                let hasHashtag = tests[1].first == Character("#")
                let string = tests[1].replacingOccurrences(of: "#", with: "")
                let isAlphaNum = string.range(of: "[^a-fA-f0-9]", options: .regularExpression) == nil

                let isValid = hasHashtag && isAlphaNum
...
//                print("pass")
            case "ecl":
                ...
                guard colors.contains(tests[1]) else {
//                    print("fail")
                    continue passportLoop
                }
...
//                print("pass")
            case "pid":
                guard tests[1].count == 9, let _ = Int(tests[1]) else {
//                    print("fail")
                    continue passportLoop
                }
